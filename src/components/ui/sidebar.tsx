    ...
    return (
      ...
      >
        <div
          data-sidebar="sidebar"
           className={cn(
            "h-full w-full flex flex-col bg-sidebar",
            side === "left" && "border-r",
            side === "right" && "border-l",
            className
          )}
...
>(({ className, onClick, ...props }, ref) => {
  ...
  return (
    <Button
      ...
      variant="ghost"
      size="icon"
      className={cn("h-10 w-10 mx-auto", className)}
      onClick={(event) => {
        onClick?.(event)
        toggleSidebar()
      }}
      {...props}
    >
      <PanelLeft className={cn("transition-transform duration-200", state === "collapsed" && "rotate-180")}/>
