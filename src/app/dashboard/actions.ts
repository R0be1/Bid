...
export async function recordPaymentAction(paymentType: PaymentType, method: 'direct' | 'receipt', receiptUrl?: string): Promise<ActionResult<User>> {
    ...
    try {
        ...
        return {
            success: true,
            message: method === 'direct' ? `Payment for ${paymentType} successful. Your account is approved.` : `Receipt for ${paymentType} submitted for review.`,
            data: userForClient
        };

    } catch (error) {
        console.error("recordPaymentAction error:", error);
        return { success: false, message: "An unexpected error occurred while recording payment." };
    }
}
